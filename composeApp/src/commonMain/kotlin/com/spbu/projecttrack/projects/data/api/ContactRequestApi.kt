...
import com.spbu.projecttrack.projects.data.model.EmailRequest
...
class ContactRequestApi(private val client: HttpClient) {
    ...
    suspend fun sendFeedback(
        ...
        email: String,
        message: String,
        subject: String = "ОБРАТНАЯ СВЯЗЬ CITEC",
        to: String = "sasha-uhnov@yandex.ru",
    ): Result<Unit> {
        return try {
            val endpoint = ApiConfig.Public.EMAIL_SEND_REQUEST
            val url = "${ApiConfig.baseUrl}$endpoint"

            AppLog.d(logTag, "POST $url (feedback)")
            AppLog.d(logTag, "Body: name=$name, email=$email, subject=$subject, to=$to")
            ...
            AppLog.e(logTag, "Feedback request failed", e)
            Result.failure(e)
        }
    }
}
