...
- `itclinicapp-debug.apk`
- `android-update.json`

The Android application uses that rolling prerelease channel to check whether a newer main-branch build is available and, if needed, offers the user an update link.
...
The Android application checks the GitHub Releases API for the newest prerelease whose tag starts with:
...
Verify that the default-branch CD run created a prerelease and uploaded both:
...
- the installed APK version code must be lower than the latest published main-branch build
...
- Android in-app updates depend on public GitHub Releases metadata, not on a dedicated backend service.
